...
//foreach (var line in input.Replace("Sue", "").Split(Environment.NewLine))
//{
//    var split = line.Split(", ");
//    var first = split[0].SkipWhile(x => x != ':').Skip(2);
//    var all = split.Skip(1).Concat(new[] { new string(first.ToArray()) });
//    if (all.All(x => detection.Contains(x)))
//    {
